    ...
    <?php include("inc/database.php"); ?>
     ...
     <?php
      if(isset($_POST['action']))
      {
		@session_start();
        $id=$_POST['action'];
        $requete='SELECT * FROM scp WHERE id=' . $id;
        $exec=mysql_query($requete);
        $ligne=mysql_fetch_array($exec);
        {
         $name=$ligne['name'];
         $class=$ligne['class'];
         $accreditation=$ligne['accreditation'];
		...
		$requete='SELECT * FROM user WHERE id=' . $idplayer;
		$exec=mysql_query($requete);
		$ligne=mysql_fetch_array($exec);
		{
		 $level=$ligne['Level'];
                                    ...
                                    <table class="date" border="0" cellpadding="0" cellspacing="0">
                                        <tbody><tr>
                                            ...
                                            <td style="color: #fefefe; font-size: 11px; font-weight: normal; font-family: Helvetica, Arial, sans-serif;">
                                                <singleline>
                                                    <?php echo "ACCREDITATION: " . $accreditation; ?>
                                                </singleline>
                                            </td>
                                        </tr>
